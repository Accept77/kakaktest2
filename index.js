...
async function parseSheetRecords(tabName, label) {
  ...
  for (let i = 0; i < rows.length; i++) {
    for (let j = 0; j < rows[i].length; j++) {
      ...
      let price = '', plan = '', extras = '';
      for (let k = i+1; k < rows.length; k++) {
        const labelCell = (rows[k][j]   || '').toString().trim();
        const valCell   = (rows[k][j+1] || '').toString().trim().replace(/원$/,'');
        if (!labelCell) break;
        if (/출고가|MNP전환지원금|실제\s*구매가격/.test(labelCell)) price = valCell;
        else if (/요금제/.test(labelCell))                        plan   = valCell;
        else if (/부가서비스/.test(labelCell))                    extras = valCell;
        else break;
      }

      recs.push({ sheet: label, modelRaw, modelNorm, capacity, telecom, type, price, plan, extras });
    }
  }
...
function parseQuestion(q) {
  const m = q.match(/(갤럭시|아이폰)\s*(\S*?)\s+(\d+)(?:GB)?\s*(SKT|KT|LG)?/i);
  if (!m) throw new Error('“갤럭시/아이폰 + 용량 [+ 통신사]” 형식으로 질문해주세요.');
  const modelNorm = (m[1] + (m[2]||'')).replace(/\s+/g,'').toLowerCase();
  const capacity  = m[3];
  const operator  = m[4]?.toUpperCase();
  console.log(`▶ 질문 파싱 -> model: ${modelNorm}, capacity: ${capacity}, operator: ${operator||'전체'}`);
...
function findBestModel(modelNorm, candidates) {
  ...
  console.log(`▶ fuzzy 매칭 -> bestModel: ${bestMatch.target} (${bestMatch.rating})`);
...
async function formatWithGPT(question, onMatches, inMatches) {
  const block = (label, arr) => {
    ...
    return `✅ ${label} 가격 조건:\n` +
      arr.map(r =>
        `- ${r.telecom} ${r.type}: 할부원금 ${r.price}원, 요금제 ${r.plan}원, 부가서비스 ${r.extras||'없음'}`
      ).join('\n')
    + '\n\n';
  };
...
고객 질문: ${question}
  ...
  const res = await openai.chat.completions.create({
    model: 'gpt-3.5-turbo',
    messages: [
      { role: 'system', content: '당신은 숙련된 휴대폰 요금 상담 챗봇입니다. 데이터만 참고하세요.' },
...
(async() => {
  try {
    ...
    const availableModels = Array.from(new Set(allRecs.map(r => r.modelNorm)));
    ...
    const answer = await formatWithGPT(question, onlineMatches, instoreMatches);
